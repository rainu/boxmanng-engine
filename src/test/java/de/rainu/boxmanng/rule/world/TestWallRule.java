...
public class TestWallRule {
	...
	@Before
	public void init(){
		toTest = spy(new WallRule(mockedWorld));
	}
	...
	public void checkCoord(){
		...
		elements.add(new Player(0));
		
		doReturn(elements).when(mockedWorld).get(any(WorldCoord.class));
		try{
			toTest.checkCoord(coord);
			fail("It should be thrown an exception.");
		}catch(RuleException e){}
		
		elements.add(new Mark());
		try{
			toTest.checkCoord(coord);
			fail("It should be thrown an exception.");
		}catch(RuleException e){}
		
		doReturn(Collections.EMPTY_LIST).when(mockedWorld).get(any(WorldCoord.class));
		try{
			toTest.checkCoord(coord);
		}catch(RuleException e){
			e.printStackTrace();
